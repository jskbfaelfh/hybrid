...
def recalculate_customer_financials(customer_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        # Sum components
        res = cursor.execute('SELECT SUM(total_price) as sum FROM customer_components WHERE customer_id = ?', (customer_id,)).fetchone()
        total_price = float(res['sum']) if res['sum'] else 0.0
        
        # Check if financial status exists
        fin = cursor.execute('SELECT * FROM financial_status WHERE customer_id = ?', (customer_id,)).fetchone()
        if fin:
            down_payment = float(fin['down_payment'])
            remaining_balance = max(0.0, total_price - down_payment)
            cursor.execute('''
                UPDATE financial_status
                SET total_price = ?, remaining_balance = ?
                WHERE customer_id = ?
            ''', (total_price, remaining_balance, customer_id))
        else:
            cursor.execute('''
                INSERT INTO financial_status (customer_id, total_price, down_payment, remaining_balance, total_installments)
                VALUES (?, ?, 0.0, ?, 0)
            ''', (customer_id, total_price, total_price))
        conn.commit()
    except Exception as e:
        print(f"Error recalculating financials: {e}")
        conn.rollback()
    finally:
        conn.close()
...
def add_component(id):
    ...
    recalculate_customer_financials(id)
...
def delete_component(comp_id):
    ...
    cursor = conn.cursor()
    try:
        comp = cursor.execute('SELECT customer_id FROM customer_components WHERE id = ?', (comp_id,)).fetchone()
        if comp:
            cust_id = comp['customer_id']
            cursor.execute('DELETE FROM customer_components WHERE id = ?', (comp_id,))
            conn.commit()
            conn.close()
            recalculate_customer_financials(cust_id)
        else:
            conn.close()
    except Exception as e:
        conn.rollback()
...
def save_financials(id):
    ...
    cursor = conn.cursor()
    try:
        # Fetch dynamic total price from components
        res = cursor.execute('SELECT SUM(total_price) as sum FROM customer_components WHERE customer_id = ?', (id,)).fetchone()
        total_price = float(res['sum']) if res['sum'] else 0.0
        
        remaining_balance = total_price - down_payment
        if remaining_balance < 0:
            conn.close()
            return jsonify({'error': 'الدفعة الأولى لا يمكن أن تكون أكبر من المبلغ الكلي للأنظمة المجهزة'}), 400
...
def get_dashboard_stats():
    ...
    # 6. Check for overdue installments in database and update their status to Overdue if due_date is in the past
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    # Fetch unpaid installments due ONLY in the current month
    unpaid_list = query_db('''
        SELECT i.*, c.name as customer_name, c.phone as customer_phone
        FROM installments i
        JOIN customers c ON i.customer_id = c.id
        WHERE i.status = 'Unpaid' AND strftime('%Y-%m', i.due_date) = ?
    ...
    # 7. Chart Data (Monthly collections: Down payments in the installation month + paid installments in the payment month)
    chart_sales_rows = query_db('''
        SELECT month, SUM(amount) as total
        FROM (
            SELECT strftime('%Y-%m', COALESCE(NULLIF(c.installation_date, ''), c.created_at)) as month, f.down_payment as amount
            FROM customers c
            ...
            SELECT strftime('%Y-%m', COALESCE(NULLIF(i.paid_date, ''), CURRENT_DATE)) as month, i.amount as amount
            FROM installments i
            WHERE i.status = 'Paid'
        )
        GROUP BY month
    ...
    chart_expenses_rows = query_db('''
        SELECT strftime('%Y-%m', COALESCE(NULLIF(expense_date, ''), CURRENT_DATE)) as month, SUM(amount) as total
...
def compile_documents(id):
    ...
    vars_map = {
        ...
        '{طريقة_التعاقد}': 'أقساط شهرية' if c.get('sale_type') == 'Installment' else 'نقد كاش',
...
def save_r2_settings():
    ...
    return jsonify({'success': True, 'message': 'تم حفظ إعدادات Cloudflare R2 وتحديث حالة التفعيل بنجاح'})
...
def manual_r2_backup():
    ...
    if res.get('success'):
        # Update database with last backup time
        conn = get_db_connection()
        now_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        conn.execute("INSERT OR REPLACE INTO settings (key, value) VALUES ('r2_last_backup_time', ?)", (now_str,))
        conn.commit()
        conn.close()
        return jsonify({'success': True, 'message': f'تم رفع النسخة الاحتياطية بنجاح إلى Cloudflare R2 باسم: {res.get("filename")}'})
    else:
